...
per_part = 250
for part, part_start in enumerate(range(0, len(img_sample), per_part)):

    print('part', part, 'from', part_start, 'to', part_start+per_part)

    # extract all descriptors
    descriptors = []
    for i, (img_array, mask_array, amap_array, seg_label) in enumerate(zip(img_sample[part_start:part_start+per_part, ...],
                                                                           mask_sample[part_start:part_start+per_part, ...],
                                                                           activation_maps[part_start:part_start+per_part, ...],
                                                                           segmentation_labels[part_start:part_start+per_part, ...])):
        print('part', part, 'img:', i)
